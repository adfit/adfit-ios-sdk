...
* (추가적으로 아래의 코드에 대하여 설명한다면 표시할 뷰의 너비에서 설정된 좌우 여백을 빼서 비즈보드 너비를 계산합니다. <br>
 비즈보드 비율은 1029 / 258 값으로 설정 합니다. (1029 / 258 의 비율은 비즈보드에 표시되는 원본 이미지의 비율입니다. 고정값으로 사용하시면 됩니다.) <br>
 비즈보드 너비에서 비즈보드 비율을 나눠서 비즈보드 높이값을 계산 합니다.  <br>
 비즈보드 높이값에 설정된 상하여백을 더해준 값을 표시할뷰의 높이로 설정 하시면 됩니다. 
)
